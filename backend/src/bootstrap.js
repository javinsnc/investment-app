...
async function recomputeCurrentAssets() {
  ...
  const { rows: ops } = await db.query(`
    SELECT name, ticker, asset_type, side, op_date, price::numeric AS price, quantity::numeric AS quantity
    FROM operations ORDER BY op_date ASC, id ASC;
  `);
  const map = new Map();
  for (const o of ops) {
    const key = o.ticker;
    ...
    if (o.side === "BUY") {
      ...
      cur.avg = newAvg; cur.qty = newQty;
    } else if (o.side === "SELL") {
      cur.qty = cur.qty - Number(o.quantity);
      if (cur.qty < 0) cur.qty = 0;
    }
    cur.name = cur.name || o.name; cur.asset_type = cur.asset_type || o.asset_type;
    map.set(key, cur);
  }
      ...
      await db.query(
        `INSERT INTO current_assets (name, ticker, asset_type, average_price, quantity)
         ...
         ON CONFLICT (ticker) DO UPDATE SET
           ...
           average_price=EXCLUDED.average_price, quantity=EXCLUDED.quantity;`,
